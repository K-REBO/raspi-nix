#!/usr/bin/env bash
# Obsidian Self-Hosted LiveSync 初期設定スクリプト
#
...
show_deploy_instructions() {
    info "設定の準備が完了しました。"
    echo
    warning "重要: Raspberry Pi 上でビルドすると時間がかかります。"
    warning "ホストマシンからクロスビルド + デプロイを実行してください。"
    echo
    echo "【手順1】configuration.nix でサービスを有効化:"
    echo "  vim ${SCRIPT_DIR}/configuration.nix"
    echo "  以下を変更:"
    echo "    services.obsidian-livesync.enable = false; → true"
    echo "    services.obsidian-tunnel.enable = false; → true"
    echo
    echo "【手順2】変更をコミット (git を使用している場合):"
    echo "  git add ${SCRIPT_DIR}"
    echo "  git commit -m 'Enable Obsidian LiveSync services'"
    echo "  git push"
    echo
    echo "【手順3】ホストマシンから deploy-rs でデプロイ:"
    echo "  (ホストマシンで実行)"
    echo "  cd /path/to/raspi-nix"
    echo "  nix run github:serokell/deploy-rs -- .#nixpi"
    echo
}
...
main() {
    ...
    success "セットアップが完了しました!"
    warning "ホストマシンから deploy を実行して設定を適用してください。"
}
